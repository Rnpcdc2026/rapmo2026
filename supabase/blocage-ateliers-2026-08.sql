-- Garde-fou ateliers du vendredi matin (demande Nadia/Eric, août 2026)
-- 1) Jauge 132 par atelier (= 2 sessions x 66) sur les 4 ateliers
-- 2) La fonction register_participant bloque désormais un atelier complet (code P0004)

-- ── 1. Jauges à 132 ──
update workshops set capacity = 132
where code in (
  'atelier-piece-toit',
  'atelier-biodiversite',
  'atelier-optimisation-travaux',
  'atelier-projet-strategique'
);

-- ── 2. Fonction avec contrôle de capacité des ateliers ──
...
BEGIN
  ...
  IF p_friday_visit_id IS NOT NULL THEN
    ...
    END IF;
  END IF;
  -- 2ter. Lock + check quota ateliers (vendredi matin)
  IF p_workshop_ids IS NOT NULL AND array_length(p_workshop_ids, 1) > 0 THEN
    FOREACH v_workshop_id IN ARRAY p_workshop_ids LOOP
      SELECT capacity INTO v_ws_capacity
      FROM workshops WHERE id = v_workshop_id FOR UPDATE;
      IF v_ws_capacity IS NOT NULL AND v_ws_capacity < 999 THEN
        SELECT COUNT(*) INTO v_ws_taken
        FROM registration_workshops WHERE workshop_id = v_workshop_id;
        IF v_ws_taken >= v_ws_capacity THEN
          RAISE EXCEPTION 'Atelier complet: %', v_workshop_id
            USING ERRCODE = 'P0004';
        END IF;
      END IF;
    END LOOP;
...
-- Vérification
select code, title, capacity from workshops order by display_order;
